				...
				<div class="col-xs-12 col-sm-3 center">
					<div>
						<span class="profile-picture">
							<?php 
							$file_foto = file_exists(base_url('./uploads/user/'.$foto));
							if (!$file_foto && !empty($foto)) {
								$src = base_url('./uploads/user/'.$foto);
							}else{
								$src = base_url('fileAdmin/images/avatar.png');
							}
							?>
	...
	$(document).ready(function() {
		...
		$(".input-sm").focusout(function(){
			...
			$.ajax({
				...
				dataType : "JSON",
				data : {
					'id' : id, 'field' : field, 'value' : value, dataJson
				},
