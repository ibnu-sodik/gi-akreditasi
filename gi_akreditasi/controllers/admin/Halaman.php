...
class Halaman extends CI_Controller {
	...
	public function hapus($id_halaman)
	{
		...
		$text = '<strong>'.$data->nama_halaman.'</strong> Berhasil dihapus dari halaman.!';
		$this->halaman_model->hapus_halaman($id_halaman);
		// 0=login, 1=logout, 2=create, 3=update, 4=delete, 5=resetPass, 6=reply
		$this->log_model->save_log($this->session->userdata('id_login'), 4, 'Menghapus halaman '.$data->nama_halaman);
	...
	public function edit($id_halaman)
	{
		...
		$data['url'] 				= site_url('admin/halaman/edit/'.$id_halaman);
		$data['canonical'] 			= site_url('admin/halaman/edit/'.$id_halaman);
	...
	public function tambah()
	{
		$site 						= $this->site_model->get_site_data()->row_array();
		$data['csrf_token'] 		= $this->security->get_csrf_hash();
		$data['url'] 				= site_url('admin/halaman/tambah');
		$data['canonical'] 			= site_url('admin/halaman/tambah');
		$data['site_title'] 		= $site['site_title'];
		$data['site_name'] 			= $site['site_name'];
		$data['site_keywords'] 		= $site['site_keywords'];
		$data['site_author'] 		= $site['site_author'];
	...
	public function index()
	{
		$site 						= $this->site_model->get_site_data()->row_array();
		$data['csrf_token'] 		= $this->security->get_csrf_hash();
		$data['url'] 				= site_url('admin/halaman');
		$data['canonical'] 			= site_url('admin/halaman');
		$data['site_title'] 		= $site['site_title'];
		$data['site_name'] 			= $site['site_name'];
		$data['site_keywords'] 		= $site['site_keywords'];
		$data['site_author'] 		= $site['site_author'];
...
/* Location: .//C/laragon/www/gi-akreditasi/gi_akreditasi/controllers/admin/Halaman.php */
