...
class Reset extends CI_Controller {
	...
	public function update_password($token)
	{
		...
		if ($this->form_validation->run() === FALSE) {
			...
			$opsi = ['cost'=>12];
			$password = password_hash($pass_baru, PASSWORD_BCRYPT, $opsi);
			$this->reset_model->update_password($password, $email);
	...
	public function send_code_to_email($email, $reset_code)
	{
		...
		$dareq = $this->reset_model->get_data_personil_by_email($email)->row_array();
...
/* Location: .//C/laragon/www/gi-akreditasi/gi_akreditasi/controllers/admin/Reset.php */
